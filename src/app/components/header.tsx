  ...
  return (
    <header className="bg-white shadow-md p-4 flex justify-between items-center relative border-b-2 border-gray-200">
      <div className="flex items-center space-x-4">
        <Image src="/starbuckslogo.png" alt="Company Logo" width={50} height={50} />
      </div>
      ...
      <GiHamburgerMenu onClick={() => setMenuOpen(!menuOpen)} className="text-black text-3xl mr-2 md:hidden"/>
      {menuOpen && (
        <div className="absolute top-full left-0 text-black w-full bg-white shadow-md p-4 flex flex-col space-y-4 md:hidden">
          ...
          <Link href="/coffeehouse" onClick={() => setMenuOpen(false)}>Coffeehouse</Link>
          <Link href="/services" onClick={() => setMenuOpen(false)}>Services</Link>
          <Link href="/rewards" onClick={() => setMenuOpen(false)}>Starbucks Rewards</Link>
